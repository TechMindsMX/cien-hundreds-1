...
<div class="form-group fieldcontain ${hasErrors(bean: model, field: 'role', 'error')} required">
    ...
    <div class="${session.inputWidth}">
      <g:select value="model.role" name="role" from="${Role.findAll{authority=='ROLE_FACILITATOR' || authority=='ROLE_BUYER'}}" optionKey="authority" optionValue="authority" class="form-control"/>
    </div>
</div>
