			...
			<ul class="property-list activity">
				...
				<li class="fieldcontain">
					<span id="musician-label" class="${session.labelWidth} property-label"><g:message code="musician.label" default="Musician" /></span>
						...
						<span class="property-value" aria-labelledby="musician-label"><g:link controller="musician" action="show" params="['uuid' : activityInstance?.musician?.uuid]">${activityInstance?.musician?.name?.encodeAsHTML()}</g:link></span>
